...
The `QueryRow` provides access to row data and implements `AutoCloseable`:
...
public interface QueryRow extends AutoCloseable {
    ...
    // Releases native resources held by this row (idempotent, never throws)
    @Override
    default void close() {}
}
----
...
[NOTE]
====
The template closes each row automatically once your `RowMapper` finishes.
Values read from a row must not be used after it is closed. If you keep a row
beyond the mapping callback, close it yourself — `QueryRow` works with
try-with-resources.
====
